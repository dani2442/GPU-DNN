...
private:
    ...
    void backprop(Matrix& input, TargetType& target)
    {
        const int nlayer = num_layers();
        if(nlayer <= 0)
            return;
...
public:
	...
	void set_default_callback() { m_callback = &m_default_callback; }
	void set_output(Output* output) { if (m_output) delete m_output; m_output = output; }
    ...
    ///
    void init(const float& mu = 0.f, const float& sigma = 0.01f, int seed = -1)
    {
        check_unit_sizes();
    ...
    void check_gradient(const Matrix& input, const TargetType& target, int npoints, int seed = -1)
    {
        ...
        for(int i = 0; i < npoints; i++)
        {
            ...
            const int layer_id = int((float)rand()/RAND_MAX * nlayer);
            ...
            const int param_id = int((float)rand()/RAND_MAX * nparam);
            ...
            this->forward(input);
            this->backprop(input, target);
            const float loss_pre = m_output->loss();

            param[layer_id][param_id] += eps * 2;
            this->set_parameters(param);
            this->forward(input);
            this->backprop(input, target);
            const float loss_post = m_output->loss();
